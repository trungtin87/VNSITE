...
Cú pháp hỗ trợ:
  ...
  {{ x | ten_loc }} / {{ x | loc: "doi_so" }} / {{ x | loc_1 | loc_2 }}
                                        -> chèn biến qua bộ lọc (xem loc.py), nối được nhiều lọc
...
_RE_BIEN = re.compile(r'{{\s*([\w.]+)((?:\s*\|\s*\w+(?:\s*:\s*"[^"]*")?)*)\s*}}')
_RE_1_LOC = re.compile(r'\|\s*(\w+)(?:\s*:\s*"([^"]*)")?')
...
class BoMayTemplate:
    ...
    def _thay_bien(self, html: str, context: dict) -> str:
        def _thay(m):
            duong_dan_bien, chuoi_loc = m.group(1), m.group(2)
            gia_tri = self._lay_theo_duong_dan(context, duong_dan_bien)
            for m_loc in _RE_1_LOC.finditer(chuoi_loc):
                ten_loc, doi_so = m_loc.group(1), m_loc.group(2)
                ham_loc = BO_LOC.get(ten_loc)
                if ham_loc is None:
                    continue  # tên lọc không tồn tại -> bỏ qua, không crash
                gia_tri = ham_loc(gia_tri, doi_so, context)
